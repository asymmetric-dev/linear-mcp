...
(hasPatCredentials || hasOAuthCredentials ? describe : describe.skip)(
  "LinearAuth Integration",
  () => {
    // PAT Authentication Tests
    (hasPatCredentials ? describe : describe.skip)(
    ...
    (hasOAuthCredentials ? describe : describe.skip)("OAuth Flow", () => {
      ...
      it("should generate valid authorization URL", () => {
        ...
        expect(url).toContain(
          `redirect_uri=${encodeURIComponent(process.env.LINEAR_REDIRECT_URI!)}`,
        );
        expect(url).toContain("response_type=code");
        expect(url).toContain(
          "scope=read%2Cwrite%2Cissues%3Acreate%2Coffline_access",
        );
      ...
      const hasTokens =
        process.env.LINEAR_AUTH_CODE && process.env.LINEAR_REFRESH_TOKEN;
      (hasTokens ? it : it.skip)(
        "should exchange auth code for tokens",
        async () => {
          const authCode = process.env.LINEAR_AUTH_CODE;
          if (!authCode) {
            throw new Error(
              "LINEAR_AUTH_CODE environment variable is required",
            );
          }
      ...
      (hasTokens ? it : it.skip)(
        "should make authenticated requests with OAuth token",
        async () => {
          // Set initial token data (requires valid refresh token)
          const refreshToken = process.env.LINEAR_REFRESH_TOKEN;
          if (!refreshToken) {
            throw new Error(
              "LINEAR_REFRESH_TOKEN environment variable is required",
            );
          }
